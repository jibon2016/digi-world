<script setup>
import { ref, onMounted} from 'vue';
import axios from 'axios'
import TableCore from "@/components/TableCore.vue";

const items = ref([])
const headers = ref([])
const loading = ref(false)

headers.value =  [
          { field: 'id',title: 'ID', isUnique: true, sort:false, type: 'number'},
          { field: 'firstName', title: 'User Name'},
          { field: 'age', title: 'Age', type: "number" },
          { field: 'email', title: 'email' },
          { field: 'image', title: 'image', },
          { field: 'gender', title: 'Gender'},
          { field: 'actions', title: 'Actions', sort: false },
        ]
onMounted( async () => {
    const response = await axios.get('https://dummyjson.com/users')
    .then(function (response) {
        items.value = response.data.users;
        loading.value = true
  }).catch((error) => {
    console.log(error);
  })
});

function editValue(data){
  window.location.href = window.location.origin + '/user/edit/'+ data.id
}
function deleteValue(data){
  window.location.href = window.location.origin + '/user/delete/'+ data.id
}

</script>

<template>
    <div class="bg-white p-5">
        <TableCore v-if="loading" :items=items :headers=headers @handle-edit="editValue" @handle-delete="deleteValue" />
    </div>
</template>
